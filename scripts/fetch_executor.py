...
# 常见正文容器（按优先级）
CONTAINERS = ['v_news_content', 'vsb_content', 'article-content', 'article', 'content',
              'js_content', 'wp_articlecontent', 'news_content', 'TextContent', 'main',
              'entry-content', 'post-content', 'text', 'detail', 'show-content']


def extract_main_body(html):
    """从 HTML 提取正文文本（多容器回退 + 微信专用 + 通用兜底）"""
    # 微信文章专用: js_content
    m = re.search(r'<div[^>]*class=["\'][^"\']*js_content[^"\']*["\'][^>]*>([\s\S]*?)</div>\s*(?:<script|<div[^>]*id=["\']js_pc_)', html)
    if not m:
        m = re.search(r'<div[^>]*id=["\']js_content["\'][^>]*>([\s\S]*?)<div[^>]*id=["\']js_sg_bar', html)
    if not m:
    ...
    if m:
        ...
        for cls in CONTAINERS:
            m = re.search(r'<(?:div|section|article)[^>]*?(?:class|id)=["\'][^"\']*'
                          rf'{cls}[^"\']*["\'][^>]*>([\s\S]*?)</(?:div|section|article)>', html, re.I)
            if m:
                frag = m.group(1)
                break
        if frag is None:
            ...
            frag = html
    # 去掉脚本/样式/注释
    frag = re.sub(r'<script[\s\S]*?</script>|<style[\s\S]*?</style>|<!--[\s\S]*?-->', ' ', frag)
    frag = re.sub(r'<(br|/p|/div|/li|/h[1-6]|/tr)[^>]*>', '\n', frag, flags=re.I)
    frag = re.sub(r'<[^>]+>', ' ', frag)
    frag = re.sub(r'[ \t\xa0]+', ' ', frag)
    frag = re.sub(r'\n\s*\n+', '\n', frag)
    # 去常见页脚噪声
    frag = re.sub(r'(责任编辑|审核|发布人|编辑|阅读原文|Copyright|版权所有)[：:].*', '', frag)
    return frag.strip()
    ...
    if retry_failed:
        ...
        try:
            with open(out_path, 'r', encoding='utf-8') as f:
                lines = [l for l in f if not (
                    (lambda r: r.get('source_id') in failed_ids)(json.loads(l)) if l.strip() else False)]
    ...
    with out:
        for i, t in enumerate(tasks):
            ...
            if (i + 1) % 10 == 0 or i == len(tasks) - 1:
                print(f'  进度 {i + 1}/{len(tasks)} (跳过 {skipped}, 写出 {results_written})')
            time.sleep(INTERVAL)
