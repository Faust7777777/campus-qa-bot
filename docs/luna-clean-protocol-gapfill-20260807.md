...
## Gapfill handoff override (2026-08-07)

This batch is the 74-source gapfill described in `docs/handoff-luna-extract-20260807.md`.
That handoff takes precedence over generic scope heuristics below:

- Use the input `seed_query` as the target question. If `clean_text` answers it, extract a
  fact card even when the title says “通知”, “实施办法”, or names a recommendation event.
- “保研/推免” in this batch means undergraduate recommendation to graduate school and is
  in scope. Do not exclude it as graduate-only.
- Do not mark a source `out_of_scope` merely because it is a school notice, a registration
  notice, or a dated procedure. Mark it out of scope only when it is clearly an event report,
  lecture/meeting advertisement, unrelated news, staff/graduate-only service, or non-campus
  noise.
- Evidence must still be an exact substring of `clean_text`; never use `seed_description` as
  evidence. If the query is not answerable from the text, use a navigation card or no card.
- For fact cards, validity must be `current` or `historical`, never `unknown`. Use `current`
  for standing procedures, eligibility, materials, contacts, locations, service entry points,
  rules, and general safety/usage guidance. Use `historical` only for a closed dated cycle,
  expired deadline, one-time event, or clearly superseded arrangement.
- Prefer one or two focused fact cards per answerable source. Do not manufacture cards just
  to reach a count, and do not add cards for the already-saturated topics listed in the handoff.
