...
This is incremental batch B. It contains six sources plus one navigation-only source;
the previous 15-source batch is not part of this workspace. Do not merge or duplicate
the previous batch here.

Batch-specific decisions:

- The two GPA/grade-management sources overlap. Extract one GPA formula fact card from
  the source with the more complete authoritative wording. If the formulas materially
  differ (general regulation versus undergraduate procedure), keep separate cards with
  distinct `fact_key` values; do not create near-duplicate cards with the same fact key.
- The campus-card sources may contain dates from a system migration. Treat standing
  usage, recharge and binding procedures as `current` when the text describes the
  ongoing service; do not make a one-time migration announcement a current rule.
- The 304-character course-selection guide is thin. It may produce a navigation card
  for the official educational-affairs entry if the URL/entry is explicit, but do not
  invent a course-selection procedure absent from the captured text.
- The 38-character initial-password source is security-sensitive. Do not extract the
  password rule, password pattern, or fallback password as a fact card. Prefer no card
  or an evidence-free navigation card to the official login page.
- The separate `nav_only_增量B.jsonl` input is navigation-only and must not be sent
  through fact extraction. Preserve it separately for catalog/navigation handling.
