...
SELECTOR_SYSTEM_PROMPT = (
    ...
    "（正文位置写着“无正文，仅为官方入口链接”）。\n"
    "第 0 步 asking：这句话是不是有人在向别人求信息？\n"
    "  群里大量消息是同学在互相回答、闲聊或感慨，它们同样会提到转专业、社团、奖学金这些词，"
    "但没有在问任何事。以下一律 false：\n"
    "  · 陈述与断言：「校区内转专业没啥限制」「社团想加什么就加什么」「不加社团没有影响」\n"
    "  · 反问：「你们不还要选课吗」「补退选不是早就结束了吗」\n"
    "  · 建议与调侃：「那活跃的是不是考虑一下转专业到电商」\n"
    "  asking 为 false 时，picked 和 entry_points 都必须为空数组，不要再看候选。\n"
    "asking 为 true 时，继续下面两步：\n"
    ...
    '只输出 JSON：{"asking": true/false, "picked": [编号, ...], '
    '"entry_points": [编号, ...], "reason": "一句话"}'
)
